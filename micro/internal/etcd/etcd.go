...
func (e *etcdRegistry) registerNode(s *registry.Service, node *registry.Node, opts ...registry.RegisterOption) error {
	...
	s.Metadata["domain"] = defaultDomain
	node.Metadata["domain"] = defaultDomain

	e.Lock()
	// ensure the leases and registers are setup for this domain
	if _, ok := e.leases[defaultDomain]; !ok {
		e.leases[defaultDomain] = make(leases)
	}
	if _, ok := e.register[defaultDomain]; !ok {
		e.register[defaultDomain] = make(register)
	}

	// check to see if we already have a lease cached
	leaseID, ok := e.leases[defaultDomain][s.Name+node.Id]
	e.Unlock()
	...
	if !ok {
		...
		key := nodePath(defaultDomain, s.Name, node.Id)
		...
		for _, kv := range rsp.Kvs {
			if kv.Lease > 0 {
				...
				// save the info
				e.Lock()
				e.leases[defaultDomain][s.Name+node.Id] = leaseID
				e.register[defaultDomain][s.Name+node.Id] = h
				e.Unlock()
	...
	e.RLock()
	v, ok := e.register[defaultDomain][s.Name+node.Id]
	e.RUnlock()
	...
	if s.Metadata == nil {
		s.Metadata = map[string]string{"domain": defaultDomain}
	} else {
		s.Metadata["domain"] = defaultDomain
	}
	...
	if lgr != nil {
		e.leases[defaultDomain][s.Name+node.Id] = lgr.ID
	}
	e.Unlock()
...
func (e *etcdRegistry) Deregister(s *registry.Service, opts ...registry.DeregisterOption) error {
	...
	for _, node := range s.Nodes {
		...
		nodes, ok := e.register[defaultDomain]
		if ok {
			delete(nodes, s.Name+node.Id)
			e.register[defaultDomain] = nodes
		}

		// delete our lease of the service
		leases, ok := e.leases[defaultDomain]
		if ok {
			delete(leases, s.Name+node.Id)
			e.leases[defaultDomain] = leases
		}
		e.Unlock()
		...
		if _, err := e.client.Delete(ctx, nodePath(defaultDomain, s.Name, node.Id)); err != nil {
			return err
		}
	}
...
func (e *etcdRegistry) GetService(name string, opts ...registry.GetOption) ([]*registry.Service, error) {
	...
	prefix := servicePath(defaultDomain, name) + "/"
...
func (e *etcdRegistry) ListServices(opts ...registry.ListOption) ([]*registry.Service, error) {
	...
	var p string
	p = prefixWithDomain(defaultDomain)
