...
func newConsulWatcher(cr *consulRegistry, opts ...registry.WatchOption) (registry.Watcher, error) {
	...
	go func() {
		//if err := wp.RunWithClientAndHclog(cr.Client(), hclog.New(os.Stderr, "", log.LstdFlags)); err != nil {
		if err := wp.RunWithClientAndHclog(cr.Client(), hclog.New(hclog.DefaultOptions)); err != nil {
			xlog.WithError(err).WithEvent("newConsulWatcher").Error()
		}
	}()
		...
		if err == nil {
			...
			go func() {
				...
				if err := wp.RunWithClientAndHclog(cw.r.Client(), hclog.New(hclog.DefaultOptions)); err != nil {
					xlog.WithError(err).WithEvent("consulWatcher_handle").Errorln()
				}
			}()
