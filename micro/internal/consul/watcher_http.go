...
import (
	...
	reg "github.com/micro/go-micro/v2/util/registry"
)

type consulWatcherHttp struct {
	r        *consulRegistryHttp
...
func newConsulWatcherHttp(cr *consulRegistryHttp, opts ...registry.WatchOption) (registry.Watcher, error) {
	var wo registry.WatchOptions
	for _, o := range opts {
		o(&wo)
	}

	cw := &consulWatcherHttp{
		r:        cr,
		wo:       wo,
		exit:     make(chan bool),
	...
	go func() {
		//if err := wp.RunWithClientAndLogger(cr.Client(), log.New(os.Stderr, "", log.LstdFlags)); err != nil {
		if err := wp.RunWithClientAndHclog(cr.Client(), hclog.New(hclog.DefaultOptions)); err != nil {
			xlog.WithError(err).WithEvent("newConsulWatcherHttp").Errorln()
		}
	}()
	cw.wp = wp
...
func (cw *consulWatcherHttp) serviceHandler(idx uint64, data interface{}) {
	...
	for _, newService := range serviceMap {
		...
		for _, oldService := range oldServices {
			...
			if len(nodes) > 0 {
				delService := reg.CopyService(oldService)
...
func (cw *consulWatcherHttp) handle(idx uint64, data interface{}) {
	...
	for service := range services {
		...
		if err == nil {
			...
			go func() {
				//if err := wp.RunWithClientAndLogger(cw.r.Client(), log.New(os.Stderr, "", log.LstdFlags)); err != nil {
				if err := wp.RunWithClientAndHclog(cw.r.Client(), hclog.New(hclog.DefaultOptions)); err != nil {
					xlog.WithError(err).WithEvent("consulWatcherHttp_handle").Errorln()
				}
			}()
...
func (cw *consulWatcherHttp) Next() (*registry.Result, error) {
	select {
	case <-cw.exit:
...
func (cw *consulWatcherHttp) Stop() {
	select {
	case <-cw.exit:
		return
	default:
