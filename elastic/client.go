...
import (
	...
	"github.com/iooikaak/frame/elastic/config"
)
...
func (c *Client) PerformRequest(ctx context.Context, opt PerformRequestOptions) (*Response, error) {
	...
	defer func(ctx context.Context, expireTime time.Time, span opentracing.Span) {
		c.report(ctx, span, opt, resp, err, expireTime)
	}(ctx, time.Now(), c.startSpan(ctx, opt))
...
func (c *Client) startSpan(ctx context.Context, req PerformRequestOptions) (span opentracing.Span) {

	if span = opentracing.SpanFromContext(ctx); span != nil {
		span = opentracing.StartSpan("elastic-"+req.Method+":"+req.Path, opentracing.ChildOf(span.Context()))
	}

	return
}

func (c *Client) report(ctx context.Context, span opentracing.Span, req PerformRequestOptions, res *Response, err error, t time.Time) {
	...
	if span != nil {
		...
		span.SetTag(req.Method+":"+req.Path, fmt.Sprintf("condition:%+v responseCode:%d err:%v   - Took: %v", body, responseStateCode, err, execDuration))
	}
}
