...
// NewPassiveConnActor warhorse.com下层服务需建立此种连接，用于接收并处理数据
	...
	case passiveConnActor:
		...
		if sd, ok := s.md[r.GetServeMethod()]; !ok {
			...
			if err := sd.Handler(s.service, c); err != nil {
				...
				bl := len(c.Response().GetBody())
				el := len(c.Response().GetErr())
				if bl == 0 && el == 0 {
					c.JSON2(0, "success", nil)
				} else if bl > 0 {
					c.Response().SetHeader(protocol.HeaderContentLength, strconv.Itoa(bl))
				} else {
					c.Response().SetHeader(protocol.HeaderContentLength, strconv.Itoa(el))
				}
			}
