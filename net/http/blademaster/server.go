...
import (
	...
	"github.com/iooikaak/frame/conf/dsn"
	"github.com/iooikaak/frame/net/criticality"
...
func (engine *Engine) Start() error {
	...
	if err != nil {
		err = errors.Wrapf(err, "blademaster: listen tcp: %s", conf.Addr)
		return err
	}

	log.Infof("blademaster: start http listen addr: %s", conf.Addr)
	server := &http.Server{
	...
	go func() {
		if err := engine.RunServer(server, l); err != nil {
			if errors.Cause(err) == http.ErrServerClosed {
				log.Infof("blademaster: server closed")
				return
			}
