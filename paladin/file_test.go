...
func TestFileEvent(t *testing.T) {
	...
	// for file test.toml
	if err := ioutil.WriteFile(filepath.Join(path, "test.toml"), []byte(`hello`), 0644); err != nil {
		t.Error(err)
	}
	select {
	case <-timeout.C:
	...
	// for file abc.toml
	if err := ioutil.WriteFile(filepath.Join(path, "abc.toml"), []byte(`test`), 0644); err != nil {
		t.Error(err)
	}
	select {
	case <-timeout.C:
...
func TestHiddenFile(t *testing.T) {
	...
	time.Sleep(time.Millisecond)
	if err := ioutil.WriteFile(filepath.Join(path, "abc.toml"), []byte(`hello`), 0644); err != nil {
		t.Error(err)
	}
