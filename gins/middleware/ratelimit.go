...
import (
	...
	limit "github.com/iooikaak/frame/ratelimit/bbrratelimit"
	"github.com/iooikaak/frame/ratelimit/bbrratelimit/bbr"
...
func (b *RateLimiter) printStats(routePath string, limiter limit.Limiter) {
	...
	if now-atomic.LoadInt64(&b.logTime) > int64(time.Second*3) {
		...
		xlog.Infof("http.internal path:%s stat:%+v", routePath, limiter.(*bbr.BBR).Stat())
	}
}

// Limit return a bm handler func.
func (b *RateLimiter) Limit() gins.HandlerFunc {
