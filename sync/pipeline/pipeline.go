...
var (
	...
	_metricDoFunc = metric.NewHistogramVec(&metric.HistogramVecOpts{
		Namespace: _metricNamespace,
		Subsystem: _metricSubSystem,
		Name:      "do_func",
		Help:      "Histogram of Do func",
		Labels:    []string{"name"},
...
func (p *Pipeline) mergeproc(mirror bool, index int, ch <-chan *message) {
	...
	for {
		...
		if len(vals) > 0 {
			...
			now := time.Now()
			p.Do(ctx, vals)
			_metricDoFunc.Observe(int64(time.Since(now)/time.Millisecond), name)
			vals = make(map[string][]interface{}, p.config.MaxSize)
			count = 0
		}
